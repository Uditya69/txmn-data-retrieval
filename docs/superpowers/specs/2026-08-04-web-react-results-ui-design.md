...
4. **Result cards** — one per deduped document (see Data Mapping below). Each card:
   ...
   - Title: a truncated snippet/chunk-text by default (neither ES nor Milvus Instant hits carry document metadata — see Data Mapping below for the exact field-level reasoning); upgrades to a real party-name title (`otherinfo.partyname`/`otherinfo.judge`) if and only if this `doc_id` also appears in `ai_mode.citations` once AI Mode has returned
   - Metadata line: only shown when the citation upgrade above applies (nothing else is available per-card); omit court/act/section even then since they're ~0% populated in the real index per the earlier live-data audit
...
4. **Card title/metadata — corrected against actual backend shapes.** `raw_search()` (`common/es_client.py`) returns ES hits as `{doc_id, score, snippet}` only — no heading/party/court/date fields. Milvus hits are `{chunk_id, doc_id, text, score}` — also no document metadata. Neither Instant leg carries anything richer than a snippet, so the earlier draft's claim of an ES "heading/subheading/party-name" title field was wrong; no such field exists in this response. The **only** richer metadata available anywhere in the websocket payload is `ai_mode_done.citations[doc_id]`, populated by `fetch_citations()` for docs prefetched during AI Mode's synthesis step (fields: `masterinfo.citations`, `masterinfo.info.court`, `masterinfo.info.bench`, `otherinfo.judge`, `otherinfo.partyname` — nested ES source objects, present only once AI Mode has succeeded, and only for the subset of `doc_id`s it happened to cite).
   Title rule for v1: **every card's title is a truncated snippet/text** (first ~80 chars of `snippet` for ES cards, of `text` for Milvus-only cards) by default. If `ai_mode.citations[doc_id]` exists for that card (i.e. AI Mode has returned and cited this exact document), the card upgrades its title/metadata line to use `otherinfo.partyname`/`otherinfo.judge` from that citation object instead. This means: right after Instant results arrive (AI Mode still pending), all cards show snippet-derived titles; a few cards may "upgrade" to a proper party-name title once AI Mode's answer lands, if they were among its citations. This is a real, accurate description of what the data supports — not a placeholder for a backend feature that doesn't exist. (A future backend change enriching every Instant result via `fetch_citations` up front, not just AI Mode's citations, would remove this limitation — flagged as a follow-up, not part of this spec.)
