...
3. **No raw-score ranking fusion between ES and Milvus.** `doc_id` is join-only (citation lookup, filter allowlist) by default. Don't blend ES's lexical score and Milvus's cosine/BM25-distance score directly — they're on incomparable scales. Sanctioned exceptions fuse by *rank position* via RRF instead, which sidesteps the incomparable-scale problem: Instant mode's opt-in `rerank` toggle (`instant/rerank.py::rrf_merge_by_doc_id`, fuses ES + Milvus dense + Milvus sparse), and AI Mode's `ai_mode_es_boost_enabled` setting (`ai_mode/retrieve.py::_apply_es_doc_boost`, adds an ES-top-N rank-based bump to the single best-scoring Milvus chunk per `doc_id` that ES also surfaced — never injects ES-only documents, since ES is doc-level and Milvus is chunk-level). Don't extend raw-score blending elsewhere off the back of these exceptions.
