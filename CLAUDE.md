...
3. **No raw-score ranking fusion between ES and Milvus.** `doc_id` is join-only (citation lookup, filter allowlist) by default. Don't blend ES's lexical score and Milvus's cosine/BM25-distance score directly — they're on incomparable scales. Two sanctioned exceptions, both rank-based, never raw-score:
   ...
   - AI Mode's ES sparse-fallback (`ai_mode/retrieve.py::_flatten()`, for `ruling`/`act_section`/`rule_section`/`article_section`/`commentary_section` — the collections whose Milvus `sparse_vector` was dropped) ranks Milvus-native and ES-origin sparse hits *locally within their own source*, then interleaves by rank position to build the list `rrf_merge()` fuses — raw scores from the two sources are never compared. See `docs/superpowers/specs/2026-08-17-milvus-sparse-es-fallback-design.md`. **Update (2026-08-25):** this fallback is no longer unconditional — it's gated behind `common.config.Settings.milvus_sparse_enabled` (env-only, default **off**) alongside native Milvus sparse, same flag, both mechanisms together. Off (the default) means no sparse pass runs anywhere in AI Mode's `retrieve()` — dense-only. On restores both native sparse (non-gap collections) and this ES fallback (gap collections) together. Instant mode's native Milvus sparse pass shares the identical flag and default (`instant/search.py::_run_milvus`), with no fallback of its own for the gap.
