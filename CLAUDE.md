...
4. **AI Mode searches all 11 Milvus collections every query.** No intent-based collection routing. (7 original + `act_section`/`rule_section`/`article_section`/`commentary_section` added 2026-08-11 upstream; `tariff_section` exists in the pipeline schema but is parked in `_disabled_collections` — not live, don't add it here yet.)
