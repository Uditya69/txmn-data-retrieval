...
3. **No raw-score ranking fusion between ES and Milvus.** `doc_id` is join-only (citation lookup, filter allowlist) by default. Don't blend ES's lexical score and Milvus's cosine/BM25-distance score directly — they're on incomparable scales. Sanctioned exception, shared by both Instant mode's opt-in `rerank` toggle and AI Mode's `ai_mode_es_boost_enabled` setting: `rrf_utils.py::apply_es_doc_boost` — RRF-merge Milvus dense+sparse by *rank position* only, then let ES's top-N doc_ids add a further rank-based bump to the single best-scoring Milvus row per `doc_id` that ES also surfaced. Never inject an ES-only `doc_id` into the fused/reranked set — ES is doc-level, Milvus is chunk-level, and there's no chunk-level text to hand a reranker/LLM for an ES-only match, so ES-only hits are dropped from these reranked views (they still show up in Instant mode's plain, non-rerank ES panel). Don't extend raw-score blending elsewhere off the back of this exception.
4. **AI Mode searches all 7 Milvus collections every query** (`common/schemas.py::MILVUS_COLLECTIONS`). No intent-based collection routing. Note: as of 2026-08-12, `data-extraction-pipeline`'s `schemas/Milvus.json` defines 11 collections — that pipeline's schema has moved ahead of this repo's hand-synced mirror; reconcile before trusting "7" as still current.
